...
def mk_test_name(name, value, index=0):
    ...
    if not is_trivial(value) and type(value) is not list and type(value) is not dict:
        return "{0}_{1}".format(name, index)
    ...
    return des_value,re.sub(r'\W|^(?=\d)', '_', test_name)



def mk_test_desc(name, value, index=0):
    """
    Generate a new name for a test case.
    ...
    for name, func in list(cls.__dict__.items()):
        if hasattr(func, DATA_ATTR):
            for i, v in enumerate(getattr(func, DATA_ATTR)):
                ...
                test_data_docstring = mk_test_desc(name, getattr(v, "__name__", v), i)
